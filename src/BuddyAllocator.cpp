...
BuddyAllocator::BuddyAllocator(const std::size_t size)
    : Allocator(size) {
...
void *BuddyAllocator::Allocate(const std::size_t size, const std::size_t alignment) {
    (void)alignment;
    std::size_t bucket = std::max(static_cast<std::ptrdiff_t>(std::ceil(std::log2(size + sizeof(Header))) - 1 - static_cast<std::ptrdiff_t>(s_Log2Header)), static_cast<std::ptrdiff_t>(0));
...
void BuddyAllocator::Deallocate(void *ptr) {
    ...
    // Check if buddy is occupied to bail early from searching for it
